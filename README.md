...
- 登录/认证：JWT + 三角色 RBAC（业务/工程师/管理）
- 应用外壳：角色工作台（问候/动作卡/概览/最近客户/待办）+ 侧栏导航（后续模块位预留）
- 客户管理：档案 CRUD（编号 CU-静态递增）、名称/编号/行业搜索；权限——业务管理、工程师无权限、管理查看
- 报价管理：报价单（客户+项目×数量×单价明细）、状态机 草稿→发出→已落单→已转委托（转委托待委托模块上线）/可取消；权限——业务创建/发出/落单、工程师无权限、管理查看
- 设计语言：反传统 ERP（示波器波形签名、编号等宽字体、大圆角留白），设计稿见 `docs/测试&BUGS/ui-mock/`
