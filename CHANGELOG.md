...
**P3-测试任务（确认委托自动生成+任务状态机）**（T-020）：任务即工单——确认委托时按 标准项目×样品 自动生成任务（编号 `T-YYYYMMDD-流水`，类别 EMI/EMS 继承自项目）；状态机 未排程→已排程→测试中→完成，打回重测（管理触发、留原因）；开测自动联动样品「在测」+委托单「测试中」；报价明细可挂标准项目引用（名称自动回填，文本兜底）；权限：工程师执行测试、管理打回、全员可查看；pytest 68 全绿

**P3-记录模板库（模板+字段定义）**（T-019）：新增记录模板——模板号 3 位静态流水（001/002…，入记录编号第 2 段）；表单层级 1-4（手册/程序文件/作业指导书/纯记录表单）；受控状态+版本（升版=编辑后 bump）；字段定义含类型/单位/必填/判据表达式（原始记录结构化录入的地基，M4 用）；权限按设计规格 §5（业务/工程师查看、管理维护/升版）；pytest 64 全绿
